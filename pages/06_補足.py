...
def main():
    ...
    * **共通秘伝について**  
    ...
    (実際にはお目当ての秘伝がつくまで育成する必要があるため、下記は育成最小数とお考え下さい。)  
      ...
      | 4.Z-ABB×BCA | 5体 | Z-A(3)B(1)B(2) × B(5)C(4)A(3)    | Z-ABC×BAAとは親の位置を入れ替えた関係にある。 | 
      | 5.Z-ABB×BCC | 5体 | Z-A(5)B(3)B(4) × B(3, 4)C(1)C(2) | Z-ACC×BAAとは親の位置を入れ替えた関係にある。 | 
      | 6.Z-ABC×BCC | 4体 | Z-A(4)B(3)C(1) × B(3)C(1)C(2)    | Z-ACC×BCAとは親の位置を入れ替えた関係にある。 | 
